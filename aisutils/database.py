...
def checkpoint():
   ...
   print 'databasy.py::checkpoint - %s:%d: Function %s CHECKPOINT' % (__file__,f.f_lineno,f.f_code.co_name)


'''
   Tipos de ddbb soportados
'''
dbTypes=(
...
def connect(options,dbType=None):
   ...
   if dbType=='sqlite':
      ...
      sys.exit('database.py::connect - Must specify a database type')

   if verbose:
      sys.stderr.write('database.py::connect - Connected to db\n')
   return cx
